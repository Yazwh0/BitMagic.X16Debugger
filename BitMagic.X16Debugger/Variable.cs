...
internal abstract class SampleVariable : SampleObject<Variable, ValueFormat>, ISampleVariableContainer
{
    ...
    protected SampleVariable(X16Debug adapter, string name, string type)
        : base(adapter)
    {
        this.Name = name;
        this.Type = type;
...
internal class SimpleVariable : SampleVariable
{
    ...
    private readonly List<SampleVariable> children;

    #endregion

    #region Constructor

    public SimpleVariable(X16Debug adapter, string name, string type, string value)
...
internal class WrapperVariable : SampleVariable
{
    ...
    private readonly Func<IReadOnlyCollection<SampleVariable>>? childrenGetter;
    ...
    public WrapperVariable(X16Debug adapter, string name, string type, Func<string> valueGetter, Func<IReadOnlyCollection<SampleVariable>>? childrenGetter = null)
        : base(adapter, name, type)
    {
    ...
    protected override IReadOnlyCollection<SampleVariable> GetChildren(bool shouldCreate = true)
    {
        ...
        return this._children ?? Array.Empty<SampleVariable>();
    }
